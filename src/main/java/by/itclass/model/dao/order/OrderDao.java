        ...
        try (Connection cn = ConnectionManager.getConnection();
            ...
            if (resultSet.next()) {
                ...
                sb.append("<h2>Order id : ").append(orderId).append("</h2>")
                        ...
                        .append("<h2 class='underline'>You ordered :").append("</h2>")
                        .append(getItemsForReceipt(orderId))
        ...
        try (Connection cn = ConnectionManager.getConnection();
            ...
            while (rs.next()) {
                sb.append("<h2>").append(rs.getInt(QUANTITY_COL)).append(" ")
                        .append(rs.getString(NAME_COL)).append(" by ").append(rs.getDouble(PRICE_COL))
                        .append(" byn. Amount : ").append(rs.getDouble(AMOUNT_COL)).append(" byn.</h2>");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }

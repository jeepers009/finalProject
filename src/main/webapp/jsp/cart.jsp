    ...
    <c:when test="${not empty orderItems}">
        ...
        <c:forEach var="item" items="${orderItems}">
            <div class="cart-item-container">
                ...
                <h3 class="cart-text">You ordered  ${item.quantity}  ${item.item.name} by
                        ${item.item.price} byn. Amount is ${item.quantity * item.item.price}</h3>
        ...
        <div class="order-container">
            <form method="post" action="<c:url value="<%=ApplicationConstants.ORDER_CONTROLLER%>"/>">
                <input type="text" name="<%= JspConstants.ADDRESS_PARAM %>"
